...
let videoButton;
let shouPlayBtn = false;
let currentVideoId;
let playedVideo;
let playedAudio;
let appBodyExist = false;
...
function getJsonForPages(pageNumber = 0) {
    return new Promise((resolution, rejection) => {
        ...
        $.get(`https://revolutions.historyadventures.app/new_revolutions/pages/${pageNumber}.html`, (data, err) => {
            resolution(data);
            rejection(err);
        });
    });
...
function makeGallerySliderPopupsEvents() {
    ...
    gallerySliderPart1.slick(slickSliderParams);
    gallerySliderPart2.slick(slickSliderParams);
    feiGallerySlider.slick(slickSliderParams);
    khariGallerySlider.slick(slickSliderParams);
    galleryThomosaBrown.slick(slickSliderParams);
...
function showPage(id) {
    ...
    checkDeviceForVideo(elementClone);
    setTimeout(() => {
...
function checkDeviceForVideo(htmlElem) {
    const videoElem = htmlElem.find('video');
    if (videoElem[0] && !appBodyExist && !isIOSDevice()) {
        videoElem[0].removeAttribute('poster')
    }
}
...
function makeVideo(id) {
    ...
    if (appBodyExist && playedVideo || isIOSDevice() && playedVideo !== undefined) {
        getAppBodyVideo();
    } else if (appBodyExist && !playedVideo || isIOSDevice() && (playedVideo === undefined || !playedVideo)) {
        videoButton.removeClass('d-flex');
        videoButton.removeClass('active');
    }
    if (playedVideo && !appBodyExist && !isIOSDevice()) {
        lastAudioOrVideo = {
            type: 'video',
            id
        };
...
function toLeft() {
    if (!popupOpened) {
        pageIndex--;
        console.log('slide left', pageIndex)
        if (pageIndex === -1) {
            pageIndex = 81;
        }
        goToPage(pageIndex);
...
function toRight() {
    if (!popupOpened) {
        pageIndex++;
        console.log('slide right', pageIndex)
        if (pageIndex === 82) {
            pageIndex = 0;
        }
        goToPage(pageIndex);
...
audioButton.on('click', () => {
    ...
    if (currentVideoId && !appBodyExist) {
...
$('#second-3d').on('shown.bs.modal', function () {
    const html = `
        <div class="iframe-3d-container w-100 d-flex justify-content-center">
            <div class="modal-content position-relative container-3d border-0 background-black">
                <button type="button" onclick="closePopup()" class="close position-absolute text-white border rounded px-3 btn btn-outline" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">
                                <span class="d-none d-sm-inline-block">Done</span>
                    <img src="./assets/img/close-white.svg" width="20" class="h-auto d-sm-none d-inline-block" alt="">
</span>
                </button>
...
function viewportHandler(event) {
    ...
    if (event.target.scale > 1 || event.target.scale < 1) {
        pageZoom = true
    } else {
        pageZoom = false
    }
...
function startTouch(e) {
    if (!popupOpened && e.targetTouches.length == 1 && !pageZoom) {
...
function tuchSpace() {
    return tuchSpaceNumber = 0 ? tuchSpaceNumber = 80 : tuchSpaceNumber;
}
function moveTouch(e) {
    if (!popupOpened && e.targetTouches.length == 1 && !pageZoom) {
        ...
        if (Math.abs(diffX) > Math.abs(diffY) + tuchSpace()) {
...
function checkPointer() {
    if (!matchMedia('(pointer:fine)').matches) {
        document.querySelector('body').classList.add('pointer-none')
    } else {
        document.querySelector('body').classList.remove('pointer-none')
    }
}
checkPointer();
window.onresize = checkPointer;
// get window location
const queryString = window.location.href;

function getDocumentHeight() {
    let documentHeight = document.querySelector('html').clientHeight;
    let documentWidth = document.querySelector('body').offsetWidth;
    if (queryString.match('mobile')) {
        document.querySelector('body').classList.add('app-body');
        appBodyExist = true
    } else if (documentWidth < 767) {
        documentHeight = documentHeight - 40;
        document.querySelector('body').classList.add('small-device');
    }
    document.querySelector('body').style.setProperty('--document-height', documentHeight + 'px');
}
getDocumentHeight();
window.onresize = getDocumentHeight;

function getModalButtons() {
    $(document).on('click', 'button[data-toggle="modal"]', () => {
        popupOpened = true;
    })
}
getModalButtons();
...
openDropdownMobile();
function myHandler() {
    videoButton.addClass('d-flex');
    videoButton.removeClass('active');
    playMobileVideo = false;
    shouPlayBtn = false;
}
function getAppBodyVideo() {
    playedVideo.removeAttribute('autoplay');
    playedVideo.removeAttribute('loop');
    playedVideo.removeAttribute('muted');
    playedVideo.muted = false;
    playedVideo.pause();
    playedVideo.setAttribute('playsinline', true);
    videoButton.addClass('d-flex');
    playedVideo.addEventListener('ended', myHandler, false);
}
$('body').on('click', '#videoButton', () => {
    playMobileVideo = !playMobileVideo;
    if (playMobileVideo) {
        playedVideo.play();
...
$('body').on('click', 'video', function () {
    shouPlayBtn = !shouPlayBtn;
    if (playMobileVideo && shouPlayBtn) {
        videoButton.addClass('d-flex');
        playedVideo.addEventListener('ended', myHandler, false);
    } else if (playMobileVideo && !shouPlayBtn) {
        videoButton.removeClass('d-flex');
    } else {
        return false
    }
...
$(document).on('click', '.slick-slide.slick-current', function () {
    $(this).children().find('.culpe-modal__text').toggleClass('opacity');
})